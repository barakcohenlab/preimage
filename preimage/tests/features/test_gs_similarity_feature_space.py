...
class TestGenericStringSimilarityFeatureSpace(unittest2.TestCase):
    ...
    def setup_gs_weights(self):
        self.gs_weights_length_two_small_sigma_p_abb = np.array([[1., 0.5], [0.5, 1.]])
        self.gs_weights_length_two_small_sigma_p_abb_abaaa = np.array([[1.5, 0.75], [0.75, 1.5]])
        self.gs_weights_normalized_abb_abaaa = np.array(
            [[1. / sqrt(3) + 0.5 / sqrt(5), 0.5 / sqrt(3) + 0.25 / sqrt(5)],
             [0.5 / sqrt(3) + 0.25 / sqrt(5), 1. / sqrt(3) + 0.5 / sqrt(5)]])
        self.gs_weights_length_four_small_sigma_p_abb = np.array([[1., 0.5], [0.5, 1.], [0.5, 1.], [0, 0]])
        self.gs_weights_length_two_large_sigma_p_abb = np.array([[2., 2.5], [2, 2.5]])
        self.gs_weights_two_gram_length_two_small_sigma_p_abb = np.array([[2., 3, 1.25, 2]])
        self.gs_weights_two_gram_length_three_small_sigma_p_abb = np.array([[1.5, 2, 0.75, 1], [1.25, 2, 2, 3]])

    def setup_gs_kernel_mock(self):
        self.positions_small_sigma = np.eye(5, dtype=np.float64)
        self.positions_large_sigma = np.ones((5, 5), dtype=np.float64)
        self.similarity_matrix = np.array([[1, 0.5], [0.5, 1]])
    ...
    def test_small_length_small_sigma_p_compute_one_gram_weights_returns_expected_weights(self):
        ...
        gs_weights = feature_space.compute_weights(y_weights=np.array([1.]), y_length=2)

        np.testing.assert_array_equal(gs_weights, self.gs_weights_length_two_small_sigma_p_abb)
    ...
    def test_two_y_small_length_small_sigma_p_compute_one_gram_weights_returns_expected_weights(self):
        ...
        gs_weights = feature_space.compute_weights(y_weights=np.array([1., 0.5]), y_length=2)

        np.testing.assert_array_equal(gs_weights, self.gs_weights_length_two_small_sigma_p_abb_abaaa)
    ...
    def test_two_y_normalized_small_sigma_p_compute_one_gram_weights_returns_expected_weights(self):
        ...
        np.testing.assert_array_equal(gs_weights, self.gs_weights_normalized_abb_abaaa)
    ...
    def test_small_length_small_sigma_p_half_weight_compute_one_gram_weights_returns_expected_weights(self):
        ...
        np.testing.assert_array_equal(gs_weights, 0.5 * self.gs_weights_length_two_small_sigma_p_abb)

    def test_large_length_small_sigma_p_compute_one_gram_weights_returns_expected_weights(self):
        feature_space = GenericStringSimilarityFeatureSpace(self.alphabet, n=1, Y=self.abb, is_normalized=False,
                                                            gs_kernel=self.gs_kernel_mock)

        gs_weights = feature_space.compute_weights(y_weights=np.array([1.]), y_length=4)

        np.testing.assert_array_equal(gs_weights, self.gs_weights_length_four_small_sigma_p_abb)
    ...
    def test_large_length_large_sigma_p_compute_one_gram_weights_returns_n_gram_weights(self):
        ...
        np.testing.assert_array_equal(gs_weights, self.gs_weights_length_two_large_sigma_p_abb)
    ...
    def test_small_sigma_p_compute_two_gram_weights_returns_expected_weights(self):
        ...
        np.testing.assert_array_equal(gs_weights, self.gs_weights_two_gram_length_two_small_sigma_p_abb)

    def test_length_three_small_sigma_p_compute_two_gram_weights_returns_expected_weights(self):
        feature_space = GenericStringSimilarityFeatureSpace(self.alphabet, n=2, Y=self.abb, is_normalized=False,
                                                            gs_kernel=self.gs_kernel_mock)

        gs_weights = feature_space.compute_weights(y_weights=np.array([1.]), y_length=3)

        np.testing.assert_array_equal(gs_weights, self.gs_weights_two_gram_length_three_small_sigma_p_abb)
